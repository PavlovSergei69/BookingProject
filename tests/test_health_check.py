...
def test_ping(api_client):
    #Когда мы используем фикстуры дополнительно ничего не нужно импортировать
...
def test_ping_server_unavailable(api_client, mocker):
    #Проверка недоступности сервера. ПРОВЕРКА данного ответа RESPONSE.RAISE_FOR_STATUS()
...
def test_ping_wrong_method(api_client, mocker):
    #СРАВНЕНИЕ данного ответа RESPONSE.RAISE_FOR_STATUS()
...
def test_ping_internal_server_error(api_client, mocker):
    #Проверка неработоспособности сервера
    mock_response=mocker.Mock()
...
def test_ping_timeout(api_client, mocker):
    #Проверка слишком долгого таймаута
