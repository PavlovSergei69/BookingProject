...
load_dotenv()   #прогрузка всех актуальных переменных с файла .env


class APIClient:
    #использование окружающей среды: тестовой или продуктовой
    #api_clients -> environments -> env -> test or prod
    def __init__(self):
        environment_str = os.getenv('ENVIRONMENT')
        #Edit configurations-> Autodetest-> Environment variables: ENVIRONMENT=TEST, вытягивает с данного значения TEST
        try:
            environment = Environment[environment_str]
        except KeyError:
            raise ValueError(f"Unsupported environment value: {environment_str}")
            #Если мы не находимый нужный ключ, то "Неподдерживаемое значение среды"
    ...
    def get_base_url(self, environment: Environment) -> str:
        ...
        if environment == Environment.TEST:
            ...
            raise ValueError(f'Unsupported environment: {environment}') #Неподдерживаемая среда

    def get(self, endpoint, params=None, status_code=200):
        url = self.base_url + endpoint
        response = requests.get(url,headers=self.headers, params=params)
        if status_code:
            assert response.status_code == status_code  #assert=утверждаем
    ...
    def ping(self):
        with allure.step('Ping api client'):
            ...
            #Доп.проверка. Вызывает исключение: class:`HTTPError`, если таковое произошло.
    ...
    def get_booking_ids (self, params=None):
        #Передаваемый параметр по умолчанию None, если мы не передадим никакие параметры
        #так запрос и отправится(без параметров)
    ...
    def update_booking (self, booking_id, booking_data ):
        with allure.step('Updates a current booking'):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT}/{booking_id}"
    ...
    def partial_update_booking (self, booking_id, booking_data ):
        with allure.step('Updates a current booking with a partial payload'):
            url = f"{self.base_url}{Endpoints.BOOKING_ENDPOINT}/{booking_id}"
